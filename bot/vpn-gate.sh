...
#
# Частые вопросы разбирает vpn-faq.py готовыми ответами, всё остальное уходит
# боссу вместе с командой для ответа. Через LLM Версы чужой текст не проходит
# ни в каком виде: это был бы канал для инъекций, расход её подписки на каждого
# случайного и поломка чата, рассчитанного на одного собеседника.
#
...
# faq <текст> — узнал тему: 0, кладёт её в FAQ_TOPIC, ответ в FAQ_TEXT.
# Не узнал: 1. У темы «ссылка» ответа нет — его собирает вызывающий, потому
# что личную страницу человека знает только он.
faq() {
  FAQ_TOPIC=""; FAQ_TEXT=""
  local out
  out=$(python3 "$HERE/vpn-faq.py" "${1:-}" 2>/dev/null) || return 1
  FAQ_TOPIC=${out%%$'\n'*}
  FAQ_TEXT=${out#*$'\n'}
  [ "$FAQ_TEXT" = "$out" ] && FAQ_TEXT=""
  return 0
}

# relay <tg_id> <username> <текст> — вопрос, который мы не разобрали, уходит
# боссу с готовой командой для ответа. Отвечает он через бота, голосом Версы.
relay() {
  send "$BOSS" "VPN, вопрос от @${2:-?} ($1):

«$3»

Ответить: /vpn re $1 <текст>"
  log "вопрос от $1 (@$2) передан боссу: «${3:0:80}»"
}
...
' "$TG_ID" "$UNAME" "$TEXT")")
    ...
    case "$(jget "$R" action)" in
      exists)
        # У человека доступ уже есть — значит он пришёл не за ним, а с
        # вопросом. Сначала пробуем ответить сами.
        if faq "$TEXT" && [ "$FAQ_TOPIC" != "ссылка" ]; then
          send "$TG_ID" "$FAQ_TEXT"
          log "ответила $TG_ID по теме «$FAQ_TOPIC»"
        elif [ -z "$TEXT" ] || [ "$FAQ_TOPIC" = "ссылка" ]; then
...
$(jget "$R" link)"
          ...
          send "$TG_ID" "Такое сама не решу — передала Диме, он ответит сюда же ♡"
          relay "$TG_ID" "$UNAME" "$TEXT"
        fi
        ;;
      new)
        RID=$(jget "$R" req_id)
...
Нужен доступ? Напиши «нужен доступ», и я ему скажу."
        ...
        ;;
      nudge)
        # Второе и следующие сообщения без внятной просьбы. Готовые ответы тут
        # НЕ даём: почти все они начинаются с «у тебя есть личная страница», а
        # у этого человека доступа ещё нет — вышло бы враньё. Поэтому подсказка
        # и передача боссу. Дальше пятого сообщения сервис сам переводит
        # человека в silent, так что спамом это не станет.
        send "$TG_ID" "Если нужен доступ к VPN — просто напиши «нужен доступ», я передам Диме ♡"
        [ -n "$TEXT" ] && relay "$TG_ID" "$UNAME" "$TEXT"
        ;;
      silent)
...
Российские сайты, банки и Госуслуги идут мимо VPN — так и задумано, пугаться не надо."
      ...
      re)
        # Ответ босса человеку через бота. Текст берём остатком строки, а не
        # третьим полем: в ответе бывают и пробелы, и переносы.
        MSG=$(printf '%s' "$TEXT" | sed -E 's#^[[:space:]]*/vpn[[:space:]]+re[[:space:]]+[0-9]+[[:space:]]*##')
        case "$ARG" in
          ''|*[!0-9]*) send "$BOSS" "VPN: нужен Telegram-ID и текст, вот так — /vpn re 12345 попробуй другой узел"; exit 0 ;;
        esac
        [ -n "$MSG" ] || { send "$BOSS" "VPN: пустой ответ, писать нечего. /vpn re $ARG <текст>"; exit 0; }
        send "$ARG" "$MSG"
        send "$BOSS" "VPN: передала ответ $ARG ♡"
        log "босс ответил $ARG: «${MSG:0:80}»"
        ;;
...
/vpn re <id> <текст> — ответить человеку от моего лица."
        ;;

      *)
        send "$BOSS" "VPN: не поняла команду. Есть /vpn list, /vpn ok N, /vpn no N, /vpn off <id>, /vpn re <id> <текст>."
        ;;
    esac
    ;;

  *)
    echo "использование: vpn-gate.sh request <tg_id> <username> <текст> | command \"/vpn ...\"" >&2
    exit 2
    ;;
esac
exit 0
