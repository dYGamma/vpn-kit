...
# Сколько файлов должно быть — знает сам зеркальщик; спрашиваем у него, а не
# держим число в двух местах.
WANT=$(grep -cE '[a-z0-9-]+\.(srs|mrs|lst)'$'\t' /usr/local/sbin/rules-mirror.sh 2>/dev/null || echo 0)
HAVE=0
for TRY in 1 2 3; do
  /usr/local/sbin/rules-mirror.sh >/dev/null 2>&1 || true
  HAVE=$(ls /var/www/vpn-help/rules 2>/dev/null | wc -l)
  { [ "$WANT" -gt 0 ] && [ "$HAVE" -ge "$WANT" ]; } && break
  [ "$TRY" = 3 ] && break
  # Не досчитались — почти всегда это оборванная закачка с GitHub, а не
  # поломка. Молча просить человека «прогнать позже вручную» неправильно:
  # пробуем сами, с паузой, иначе упрёмся в тот же обрыв.
  say "скачалось $HAVE из ${WANT:-?} — пробую добрать (попытка $TRY из 3)"
  sleep $((TRY * 10))
done
if [ "$WANT" -gt 0 ] && [ "$HAVE" -lt "$WANT" ]; then
  say "файлов в зеркале: $HAVE из $WANT — остальные доберёт суточный таймер"
else
  say "файлов в зеркале: $HAVE"
fi
